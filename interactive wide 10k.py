...
df = pd.concat(read_files(['./experiments/wide-10krounds-5folds.pkl'
                           ]), 
...
display(shaderdots(df, 'mean_train_auc', 'mean_validation_auc', 700, 700, category_column='iteration_type'))

[display(shaderdots(df, p, 'mean_train_auc', 500, 500, x_axis_type='log', category_column='iteration_type'))
 for p in sorted(set(LOG_PARAMETERS) & set(df.columns))];

[display(shaderdots(df, p, 'mean_train_auc', 500, 500, category_column='iteration_type'))

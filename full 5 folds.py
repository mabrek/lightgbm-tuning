...
experiments = pd.concat(
    [pd.read_pickle(f'experiments/wide-5fold-3seed-full-experiments{n:03d}.pkl') for n in range(n_chunks)],
    ignore_index=True,
    sort=True)\
.groupby('experiment_id').first().reset_index()

folds = pd.concat(
    [pd.read_pickle(f'experiments/wide-5fold-3seed-full-iterations{n:03d}.pkl') for n in range(n_chunks)],
    ignore_index=True,
    sort=True)
...
splits_merged = pd.merge(
    cv_folds.loc[cv_folds.split == 1, ['experiment_id', 'param_seed', 'iteration', 'validation_auc']],
    cv_folds.loc[cv_folds.split == 3, ['experiment_id', 'param_seed', 'iteration', 'validation_auc']],
